...
component{
	...
	function preModuleLoad( event, interceptData, buffer, rc, prc ){
		// Load distributed module before ContentBox module.
		if( arguments.interceptData.moduleName eq "contentbox" ){
			controller.getModuleService().activateModule( "cbRedis" );
		}
	}
